...
export function TattooForm({ onStart, onSuccess, source }: TattooFormProps) {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const url = json.url;
      if (!url) {
        setApiError('Respuesta inválida del servidor (sin url)');
        return;
      }
      ...
      console.error(err);
      setApiError(err instanceof Error ? err.message : 'Error al generar');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="space-y-6">
        <div>
          <Label
            htmlFor="description"
            className="text-base text-gray-700 dark:text-gray-300 mb-2 block"
          >
            Describe tu tatuaje:
          </Label>
          ...
          <Textarea
            ...
            {...register('description', {
              required: 'Por favor proporciona una descripción para tu tatuaje.',
            })}
          />
